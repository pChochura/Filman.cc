...
class EkinoScraper(private val modelCache: ModelCache) {
            ...
            try {
                ...
                if (selectedUrl == null) {
                    ...
                    for (link in movieLinks) {
                        ...
                        if (itemTitle.contains(title, ignoreCase = true) ||
                            title.contains(
                                itemTitle,
                                ignoreCase = true,
                            )
    ...
    private fun parseMoviesList(element: Element): List<MovieItem> {
        val movies = mutableListOf<MovieItem>()
        ...
        element.select(".movies-list-item").forEach { item ->
            ...
            val posterUrl =
                if (posterSrc.startsWith("http") || posterSrc.isEmpty()) {
                    posterSrc
                } else {
