...
class ZaluknijScraper(
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (season != null && episode != null) {
                    ...
                    val tvShow =
                        searchResults.tvShows.firstOrNull { item ->
                            titleMatches(
                                item.titlePl,
                                title,
    ...
    private fun titleMatches(
        a: String,
        b: String,
    ): Boolean = a.contains(b, ignoreCase = true) || b.contains(a, ignoreCase = true)
    ...
    private fun parseEmbeds(doc: Document): List<EmbedLink> {
        ...
        doc.select("tr").forEach { row ->
            ...
            val serverName =
                linkTd.selectFirst("img[alt]")?.attr("alt")?.trim()
                    ?: anchor
                        .text()
                        .trim()
                        .ifEmpty { runCatching { URL(embedUrl).host }.getOrDefault("") }
